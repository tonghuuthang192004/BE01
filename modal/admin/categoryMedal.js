...
const deleteItem = async (id) => {
  const [result] = await db.query('UPDATE danh_muc SET deleted=1 where id_danh_muc=?', [id])
  return result
...
const createCategory = async (category) => {
  try {
    const query = `
      INSERT INTO danh_muc(ten, tieu_de, trang_thai, ngay_tao, hinh_anh, deleted, ngay_cap_nhat)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    return res.insertId ? { success: true, id: res.insertId } : { success: false };
  } catch (err) {
...
const getAllCategoryId= async (id)=>{
      const query="SELECT * FROM danh_muc where id_danh_muc = ?";
